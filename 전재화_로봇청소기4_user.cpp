...
void initCompMap(void)
{
    ...
    //        for (int j = 0; j < MAX; j++)
    //        {
    //            if (rotateMap[dir][i][j] == NEED_SCAN)
    //                std::cout << " ";
    //            else
    //                std::cout << rotateMap[dir][i][j];
    //        }
    //        std::cout << std::endl;
    //    }
    //}
...
bool detectStatus(bool isFirst)
{
    ...
    ++visitCheck;
    Queue<Diff> diffQ;
    diffQ.push({ 0, 0, compMap[currY][currX] });
    visit[currY][currX] = visitCheck;
