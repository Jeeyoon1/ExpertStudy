...
{
    for (int yLen = MIN_LEN; yLen < SIZE && sy+yLen < MAX && num[sy][sx] > 0; yLen++)
        for (int xLen = MIN_LEN; xLen < SIZE && sx + xLen < MAX && num[sy][sx] > 0; xLen++)
...
int recog(unsigned char image[64][64])
{
    ...
    for (int i = 1; i <= MAX - MIN_LEN; i++)
        for (int j = 1; j <= SIZE - MIN_LEN; j++)
            if(num[i][j] > 0)
                countSquare(i, j, ret);
